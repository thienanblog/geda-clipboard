...
```
Preferences are now organised into General, Clipboard, Privacy, Statistics and
About tabs. The new Statistics tab shows local copy totals for text, images and
repeated content over a day, week, month or year. These charts use bounded
on-device counters and never send or separately log clipboard content.
```
