...
func TestPopupPositionAtCursor(t *testing.T) {
	...
	p, ok := popupPosition(settings.PlacementCursor, menuBarIcon, 720, 520)
...
func TestPopupPositionAtMenuBar(t *testing.T) {
	...
	p, ok := popupPosition(settings.PlacementMenuBar, menuBarIcon, 720, 520)

	// Centred on the icon: 500 + 12 - 360 = 152, flush under the menu bar.
	if !ok || p.RelX != 152 || p.RelY != 0 {
		t.Errorf("relative = (%d, %d), ok = %v, want (152, 0), true", p.RelX, p.RelY, ok)
	}
}
...
func TestPopupPositionGlobalMatchesRelativeOnPrimary(t *testing.T) {
	...
	p, ok := popupPosition(settings.PlacementCursor, tray.Anchor{}, 720, 520)
...
func TestPopupPositionMenuBarFallsBackToCursor(t *testing.T) {
	...
	p, ok := popupPosition(settings.PlacementMenuBar, tray.Anchor{}, 720, 520)
...
func TestPopupPositionWithoutAnySource(t *testing.T) {
	...
	for _, mode := range []string{settings.PlacementCursor, settings.PlacementMenuBar} {
		if _, ok := popupPosition(mode, tray.Anchor{}, 720, 520); ok {
